...
static inline void REM_RobotGetPIDGains_set_timestamp(REM_RobotGetPIDGainsPayload *remrgpidgp, uint64_t timestamp){
    remrgpidgp->payload[4] = (timestamp >> 32);
    remrgpidgp->payload[5] = (timestamp >> 24);
    remrgpidgp->payload[6] = (timestamp >> 16);
    remrgpidgp->payload[7] = (timestamp >> 8);
    remrgpidgp->payload[8] = timestamp;
}

static inline void REM_RobotGetPIDGains_set_payloadSize(REM_RobotGetPIDGainsPayload *remrgpidgp, uint32_t payloadSize){
    remrgpidgp->payload[9] = payloadSize;
}
