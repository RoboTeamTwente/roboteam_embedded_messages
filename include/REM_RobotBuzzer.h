...
// ================================ GETTERS ================================
static inline uint32_t REM_RobotBuzzer_get_packetType(REM_RobotBuzzerPayload *remrbp){
...
static inline uint64_t REM_RobotBuzzer_get_timestamp(REM_RobotBuzzerPayload *remrbp){
    return ((uint64_t) (remrbp->payload[4]) << 40) | ((uint64_t) (remrbp->payload[5]) << 32) | ((uint32_t) (remrbp->payload[6]) << 24) | ((remrbp->payload[7]) << 16) | ((remrbp->payload[8]) << 8) | ((remrbp->payload[9]));
}

static inline uint32_t REM_RobotBuzzer_get_payloadSize(REM_RobotBuzzerPayload *remrbp){
    return ((remrbp->payload[10]));
}

static inline uint32_t REM_RobotBuzzer_get_period(REM_RobotBuzzerPayload *remrbp){
    return ((remrbp->payload[11]) << 4) | ((remrbp->payload[12] & 0b11110000) >> 4);
}

static inline float REM_RobotBuzzer_get_duration(REM_RobotBuzzerPayload *remrbp){
    uint32_t _duration = ((remrbp->payload[12] & 0b00001111) << 12) | ((remrbp->payload[13]) << 4) | ((remrbp->payload[14] & 0b11110000) >> 4);
    return (_duration * 0.0000762951094835F);
}

// ================================ SETTERS ================================
static inline void REM_RobotBuzzer_set_packetType(REM_RobotBuzzerPayload *remrbp, uint32_t packetType){
    remrbp->payload[0] = packetType;
}
...
static inline void REM_RobotBuzzer_set_timestamp(REM_RobotBuzzerPayload *remrbp, uint64_t timestamp){
    remrbp->payload[4] = (timestamp >> 40);
    remrbp->payload[5] = (timestamp >> 32);
    remrbp->payload[6] = (timestamp >> 24);
    remrbp->payload[7] = (timestamp >> 16);
    remrbp->payload[8] = (timestamp >> 8);
    remrbp->payload[9] = timestamp;
}

static inline void REM_RobotBuzzer_set_payloadSize(REM_RobotBuzzerPayload *remrbp, uint32_t payloadSize){
    remrbp->payload[10] = payloadSize;
}

static inline void REM_RobotBuzzer_set_period(REM_RobotBuzzerPayload *remrbp, uint32_t period){
    remrbp->payload[11] = (period >> 4);
    remrbp->payload[12] = ((period << 4) & 0b11110000) | (remrbp->payload[12] & 0b00001111);
}
...
static inline void REM_RobotBuzzer_set_duration(REM_RobotBuzzerPayload *remrbp, float duration){
    ...
    remrbp->payload[12] = ((_duration >> 12) & 0b00001111) | (remrbp->payload[12] & 0b11110000);
    remrbp->payload[13] = (_duration >> 4);
    remrbp->payload[14] = ((_duration << 4) & 0b11110000) | (remrbp->payload[14] & 0b00001111);
}

// ================================ ENCODE ================================
static inline void encodeREM_RobotBuzzer(REM_RobotBuzzerPayload *remrbp, REM_RobotBuzzer *remrb){
    REM_RobotBuzzer_set_packetType          (remrbp, remrb->packetType);
...
static inline void decodeREM_RobotBuzzer(REM_RobotBuzzer *remrb, REM_RobotBuzzerPayload *remrbp){
    remrb->packetType    = REM_RobotBuzzer_get_packetType(remrbp);
