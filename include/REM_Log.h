...
// ================================ GETTERS ================================
static inline uint32_t REM_Log_get_packetType(REM_LogPayload *remlp){
...
static inline uint64_t REM_Log_get_timestamp(REM_LogPayload *remlp){
    return ((uint64_t) (remlp->payload[4]) << 40) | ((uint64_t) (remlp->payload[5]) << 32) | ((uint32_t) (remlp->payload[6]) << 24) | ((remlp->payload[7]) << 16) | ((remlp->payload[8]) << 8) | ((remlp->payload[9]));
}

static inline uint32_t REM_Log_get_payloadSize(REM_LogPayload *remlp){
    return ((remlp->payload[10]));
}

// ================================ SETTERS ================================
static inline void REM_Log_set_packetType(REM_LogPayload *remlp, uint32_t packetType){
    remlp->payload[0] = packetType;
}
...
static inline void REM_Log_set_timestamp(REM_LogPayload *remlp, uint64_t timestamp){
    remlp->payload[4] = (timestamp >> 40);
    remlp->payload[5] = (timestamp >> 32);
    remlp->payload[6] = (timestamp >> 24);
    remlp->payload[7] = (timestamp >> 16);
    remlp->payload[8] = (timestamp >> 8);
    remlp->payload[9] = timestamp;
}

static inline void REM_Log_set_payloadSize(REM_LogPayload *remlp, uint32_t payloadSize){
    remlp->payload[10] = payloadSize;
}

// ================================ ENCODE ================================
static inline void encodeREM_Log(REM_LogPayload *remlp, REM_Log *reml){
    REM_Log_set_packetType          (remlp, reml->packetType);
...
static inline void decodeREM_Log(REM_Log *reml, REM_LogPayload *remlp){
    reml->packetType     = REM_Log_get_packetType(remlp);
