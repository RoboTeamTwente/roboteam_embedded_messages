...
static inline void REM_RobotAssuredPacket_set_timestamp(REM_RobotAssuredPacketPayload *remrapp, uint64_t timestamp){
    remrapp->payload[4] = (timestamp >> 32);
    remrapp->payload[5] = (timestamp >> 24);
    remrapp->payload[6] = (timestamp >> 16);
    remrapp->payload[7] = (timestamp >> 8);
    remrapp->payload[8] = timestamp;
}

static inline void REM_RobotAssuredPacket_set_payloadSize(REM_RobotAssuredPacketPayload *remrapp, uint32_t payloadSize){
    remrapp->payload[9] = payloadSize;
}
