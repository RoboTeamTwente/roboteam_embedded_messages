...
typedef struct _REM_RobotFeedback {
    ...
    float      batteryLevel        ; // float   [0.000, 30.000]      The voltage level of the battery
...
static inline float REM_RobotFeedback_get_batteryLevel(REM_RobotFeedbackPayload *remrfp){
    uint32_t _batteryLevel = ((remrfp->payload[16]) << 8) | ((remrfp->payload[17]));
    return (_batteryLevel * 0.0004577706569009F);
}
...
static inline float REM_RobotFeedback_get_ballPos(REM_RobotFeedbackPayload *remrfp){
    uint32_t _ballPos = ((remrfp->payload[18] & 0b00001111));
...
static inline void REM_RobotFeedback_set_batteryLevel(REM_RobotFeedbackPayload *remrfp, float batteryLevel){
    uint32_t _batteryLevel = (uint32_t)(batteryLevel / 0.0004577706569009F);
    remrfp->payload[16] = (_batteryLevel >> 8);
