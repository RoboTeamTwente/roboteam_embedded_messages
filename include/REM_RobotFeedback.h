...
static inline float REM_RobotFeedback_get_angle(REM_RobotFeedbackPayload *remrfp){
    uint32_t _angle = ((remrfp->payload[14]) << 8) | ((remrfp->payload[15]));
...
static inline void REM_RobotFeedback_set_timestamp(REM_RobotFeedbackPayload *remrfp, uint64_t timestamp){
    remrfp->payload[4] = (timestamp >> 32);
    remrfp->payload[5] = (timestamp >> 24);
    remrfp->payload[6] = (timestamp >> 16);
    remrfp->payload[7] = (timestamp >> 8);
    remrfp->payload[8] = timestamp;
}

static inline void REM_RobotFeedback_set_payloadSize(REM_RobotFeedbackPayload *remrfp, uint32_t payloadSize){
    remrfp->payload[9] = payloadSize;
}
...
static inline void REM_RobotFeedback_set_rho(REM_RobotFeedbackPayload *remrfp, float rho){
    ...
    remrfp->payload[10] = (_rho >> 8);
    remrfp->payload[11] = _rho;
}
...
static inline void REM_RobotFeedback_set_theta(REM_RobotFeedbackPayload *remrfp, float theta){
    ...
    remrfp->payload[12] = (_theta >> 8);
    remrfp->payload[13] = _theta;
}
