...
static inline void REM_RobotKillCommand_set_timestamp(REM_RobotKillCommandPayload *remrkcp, uint64_t timestamp){
    remrkcp->payload[4] = (timestamp >> 32);
    remrkcp->payload[5] = (timestamp >> 24);
    remrkcp->payload[6] = (timestamp >> 16);
    remrkcp->payload[7] = (timestamp >> 8);
    remrkcp->payload[8] = timestamp;
}

static inline void REM_RobotKillCommand_set_payloadSize(REM_RobotKillCommandPayload *remrkcp, uint32_t payloadSize){
    remrkcp->payload[9] = payloadSize;
}
