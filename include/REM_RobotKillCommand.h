...
static inline uint32_t REM_RobotKillCommand_get_timestamp(REM_RobotKillCommandPayload *remrkcp){
    return ((remrkcp->payload[4]) << 24) | ((remrkcp->payload[5]) << 16) | ((remrkcp->payload[6]) << 8) | ((remrkcp->payload[7]));
}

static inline uint32_t REM_RobotKillCommand_get_payloadSize(REM_RobotKillCommandPayload *remrkcp){
    return ((remrkcp->payload[8]));
}
...
static inline void REM_RobotKillCommand_set_timestamp(REM_RobotKillCommandPayload *remrkcp, uint32_t timestamp){
    remrkcp->payload[4] = (timestamp >> 24);
    remrkcp->payload[5] = (timestamp >> 16);
    remrkcp->payload[6] = (timestamp >> 8);
    remrkcp->payload[7] = timestamp;
}

static inline void REM_RobotKillCommand_set_payloadSize(REM_RobotKillCommandPayload *remrkcp, uint32_t payloadSize){
    remrkcp->payload[8] = payloadSize;
}
