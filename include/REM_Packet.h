...
static inline void REM_Packet_set_timestamp(REM_PacketPayload *rempp, uint64_t timestamp){
    rempp->payload[4] = (timestamp >> 32);
    rempp->payload[5] = (timestamp >> 24);
    rempp->payload[6] = (timestamp >> 16);
    rempp->payload[7] = (timestamp >> 8);
    rempp->payload[8] = timestamp;
}

static inline void REM_Packet_set_payloadSize(REM_PacketPayload *rempp, uint32_t payloadSize){
    rempp->payload[9] = payloadSize;
}
